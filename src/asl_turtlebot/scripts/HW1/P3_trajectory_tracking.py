...
class TrajectoryTracker:
    ...
    def compute_control(self, x, y, th, t):
        ...
        ########## Code starts here ##########
        xd = self.V_prev*np.cos(th) # approximation
        yd = self.V_prev*np.sin(th) # approximation
...
#         om = (u2*np.cos(th) - u1*np.sin(th))/self.V_prev*dt
        [dV, om] = np.linalg.inv(J).dot(np.array([u1, u2]))[:]
        V = self.V_prev + dV*dt
        ########## Code ends here ##########
