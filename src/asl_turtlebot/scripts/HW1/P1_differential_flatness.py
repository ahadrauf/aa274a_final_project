...
def compute_traj_coeffs(initial_state, final_state, tf):
    ...
    A = np.array([[1, 0, 0, 0], 
                  ...
                  [1, tf, tf**2, tf**3],
                  [0, 1, 2*tf, 3*(tf**2)]])
    bx = np.array([initial_state.x, initial_state.xd, final_state.x, final_state.xd])
    coeffs[0:4] = np.linalg.inv(A).dot(bx)
    
    by = np.array([initial_state.y, initial_state.yd, final_state.y, final_state.yd])
    coeffs[4:8] = np.linalg.inv(A).dot(by)
    ########## Code ends here ##########
    return coeffs
...
def compute_traj(coeffs, tf, N):
    ...
    Output:
        traj (np.array shape [N,7]), N points along the trajectory, from t=0
            ...
            [x, y, theta, xdot, ydot, xddot, yddot]
            Note that theta = atan2(ydot, xdot)
    """
    ...
    ########## Code starts here ##########
    # x, xdot, xddot
    traj[:,0] = coeffs[0]*np.ones(N) + coeffs[1]*t + coeffs[2]*np.power(t, 2) + coeffs[3]*np.power(t, 3)
    traj[:,3] = coeffs[1]*np.ones(N) + 2*coeffs[2]*np.power(t, 1) + 3*coeffs[3]*np.power(t, 2)
    traj[:,5] = 2*coeffs[2]*np.ones(N) + 6*coeffs[3]*t
    
    # y, ydot, yddot
    traj[:,1] = coeffs[4]*np.ones(N) + coeffs[5]*t + coeffs[6]*np.power(t, 2) + coeffs[7]*np.power(t, 3)
    traj[:,4] = coeffs[5]*np.ones(N) + 2*coeffs[6]*np.power(t, 1) + 3*coeffs[7]*np.power(t, 2)
    traj[:,6] = 2*coeffs[6]*np.ones(N) + 6*coeffs[7]*t
...
def compute_controls(traj):
    ...
    Outputs:
        ...
        d(arctan(f(x)/g(x))) = (f'(x)g(x) - f(x)g'(x))/(f(x)^2 + g(x)^2)
        d(arctan(ydot/xdot)) = (yddot*xdot - ydot*xddot)/(ydot^2 + xdot^2)
    """
    ########## Code starts here ##########
    V = np.sqrt(np.square(traj[:,3]) + np.square(traj[:,4]))
    om = (traj[:,6]*traj[:,3] - traj[:,4]*traj[:,5]) / (np.square(traj[:,4]) + np.square(traj[:,3]))
    ########## Code ends here ##########
...
def rescale_V(V, om, V_max, om_max):
    ...
    """
    ########## Code starts here ##########
#     om_clipped = np.clip(om, -self.om_max, self.om_max)
#     V_tilde = np.minimum(V, V_max*om_clipped/om)
    V_clipped = np.minimum(V_max, np.abs(V*om_max/om))
    V_tilde = np.minimum(V, V_clipped)
    ########## Code ends here ##########
    return V_tilde
...
#     plt.plot(traj_scaled[:,0], traj_scaled[:,1], 'r-',linewidth=2) ## Added afterwards for comparison - remove later
    plt.grid(True)
...
#     plt.plot(t_new, traj_scaled[:,1])
#     plt.plot(t_new, traj_scaled[:,2])
    plt.grid(True)
    plt.xlabel('Time [s]')
